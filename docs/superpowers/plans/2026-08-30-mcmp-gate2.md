...
- Resolution: a call name resolves only if exactly one document in the corpus has
  that `symbol`. Otherwise the name is discarded and recorded in
  `discarded_names`. Self-edges are dropped.
...
def test_manifest_records_documents_and_resolved_edges(tmp_path: Path) -> None:
    ...
    assert manifest.callers_by_document["alpha.helper"] == frozenset({"alpha.caller"})
